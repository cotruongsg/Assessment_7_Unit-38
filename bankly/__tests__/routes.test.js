...
describe("POST /auth/login", function() {
  ...
  // TESTS BUG #1
  test("should raise error when using jwt.decode()", async function () {
    const invalidToken = "invalid.token"; // Invalid token

    // Attempt to decode the invalid token
    try {
      jwt.decode(invalidToken);
      // If jwt.decode() does not raise an error, throw an error
      throw new Error("jwt.decode() did not raise an error");
    } catch (err) {
      // Ensure that jwt.decode() raises an error
...
describe("GET /users", function() {
  ...
  //TEST BUG #2
  test("should raise an error when user is not found", async function () {
    try {
      await User.get("nonexistent_user");
      fail("Expected User.get() to throw an error, but it didn't."); // This line should not be reached
    } catch (err) {
