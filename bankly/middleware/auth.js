...
function authUser(req, res, next) {
  try {
    ...
    if (token) {
      // we should use jwt.verify(token, SECRET_KEY)
      // jwt.decode() is a synchronous function and only
      //decodes the token without performing any verification.
      //The jwt.verify() function not only decodes the token
      // but also verifies its signature and checks for token expiration.
